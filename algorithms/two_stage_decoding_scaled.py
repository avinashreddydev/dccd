...
GSM8K_STAGE2_SYSTEM_PROMPT = (
    "You are a precise extractor. You are given a list of free-form solutions. Do the Majority Voting to get the final JSON object with the schema: "
    "{steps: string[], answer: string}. No extra text, no code fences—emit ONLY the JSON."
)
...
def combine_stage1_outputs(stage1_outputs: List[str]) -> str:
    """Combine the stage1 outputs into a single string."""
    combined_output = ""
    for i, output in enumerate(stage1_outputs):
        combined_output += f"Response {i+1}: {output}\n\n"
    return f"The following are the stage 1 outputs:\n{combined_output}"


class TwoStageDecodingScaled(BaseAlgorithm):
    """
    ...
    def __init__(
        ...
        self.stage1_sampling_params = SamplingParams(
            ...
            n=stage1_num_samples,
            stop=["\n------"]
        )
        self.alg_name = "two_stage_decoding_scaled"
    ...
    def generate_stage1(self, texts: List[str]) -> List[str]:
        ...
        for response in llm_responses:
            q_response = [o.text for o in response.outputs]
            to_stage2_prompts.append(combine_stage1_outputs(q_response))

        return to_stage2_prompts
